...
import type { ReviewResult, VocabularyEntry } from "@/lib/vocabulary/types";
...
export function VocabularyReview({ entries, mode, onReview, onExit }: { entries:VocabularyEntry[]; mode:ReviewMode; onReview:(termId:string,result:ReviewResult)=>void; onExit:()=>void }) {
  ...
  const entry = sessionEntries[index % sessionEntries.length]; const term = entry ? vocabularyById.get(entry.termId) : undefined;
  const choices = useMemo(() => { if (!term) return []; const same = vocabularyCatalog.filter((item) => item.id !== term.id && item.category === term.category); const pool = same.length >= 3 ? same : vocabularyCatalog.filter((item) => item.id !== term.id); return [term,...pool.slice(0,3)].toSorted((a,b) => stableOrder(a.id,term.id)-stableOrder(b.id,term.id)); }, [term]);
...
function ReviewAnswer({ term }: { term: NonNullable<ReturnType<typeof vocabularyById.get>> }) { return <div className="review-answer"><p>{term.definitionEn}</p><p>{term.practicalExplanation}</p><div><span>中文 — {term.definitionZh}</span><span>日本語 — {term.definitionJa}</span></div></div>; }
